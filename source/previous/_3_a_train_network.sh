...
PYTHONPATH=$PYTHONPATH:$PATH_HERE/python_datalayers

#Destination path for the files we need to write to be able to train the network
PROTOTXT_READY=$PATH_HERE/data/base_network/my_network/ready_files/ready_network.prototxt;
SOLVER_READY=$PATH_HERE/data/base_network/my_network/ready_files/ready_solver.prototx;

#Paths where we can find the original files
PROTOTXT_BASE=$PATH_HERE/base_network/my_network/base_files/googlenetbase.prototxt;
...
SNAPSHOT_PREFIX=$PATH_HERE/data/snapshots;

#Path to the folder where we can wait the initial weights in case we want to use some
INITIAL_WEIGHTS="$PATH_HERE/data/base_network/original/bvlc_googlenet.caffemodel";

#Path to the validation and train filenames
VAL_FILENAME="$PATH_HERE/data/files/filtered_val.txt";
TRAIN_FILENAME="$PATH_HERE/data/files/filtered_train.txt";
